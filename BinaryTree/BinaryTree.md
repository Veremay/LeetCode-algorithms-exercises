...
和上一道题差不多。注意，这里题目要返回的是int[]数组，但我们实际中可以用列表List，然后最后返回结果的时候处理一下就好了。

因为要找众数，所以要不断更新maxCount，一旦更新了maxCount，那List也要清空
...
![img.png](二叉搜索树最近公共祖先.png)

这道题中，和上一道题不一样的是，上一道题是需要搜索整棵树，这道题只需要搜一条边。
...
1. 准备工作
   ...
   代码先创建一个新节点（TreeNode node），里面装着要插入的数字（val）。

2. 处理空树
   
   如果树是空的（`root == null`），那么新节点直接成为树根，返回它就行。

3. 树不空时，找位置插入

   如果树不空，就拿着新数字和树根的数字比较：
   
   如果新数字比根的数字小，说明它应该放在左子树里。这时就递归调用自己（`insertIntoBST(root.left, val)`），去左子树里找合适的位置。等插入完成后，把返回的结果（可能是新的左子树根）接在根的左边（`root.left = ...`）。
   
   如果新数字比根的数字大，说明它应该放在右子树里。同样，递归调用自己去右子树里找位置，然后把返回的结果接在根的右边（`root.right = ...`）。

4. 返回根节点
   
   最后返回当前的根节点（`root`）。这样，从最开始的根一层层返回，整棵树就更新好了。
...
1. 没找到删除的点 -> 返回原树
2. 叶子节点，左空右空  -> 让其父节点指向null
3. 左孩子不为空，右孩子为空 -> 让其父节点直接指向左孩子
4. 右孩子不为空，左孩子为空 -> 让其父节点直接指向右孩子
5. 左右孩子都不为空 -> 最复杂的一种情况
   1. 让仅小于该节点的节点继位（左子树的最深的右节点），或者仅大于该节点的节点继位（右子树的最深左节点）。这里选后者。
   2. 然后把该节点的左子树放在右子树的最深左节点的左孩子处
   3. 让其父节点直接指向右孩子
...
要将一个按照升序排列的有序数组，转换为一棵高度平衡二叉搜索树。

分割点就是数组中间位置的节点。如果数组长度为偶数，中间节点有两个，那随便取哪个都行，但要保持循环不变量，即在不断分割的过程中，也会坚持左闭右闭的区间。

# 538.把二叉搜索树转换为累加树 (convertBST)

其实这道题把二叉搜索树看成有序数组就好理解很多了。
