...
# 454. 四数相加 II (foursumCount)

我们要把 $O(n^4)$ 的复杂度降下来。核心思路是：与其一次性找四个数，不如把它们分成两组。

我们将 $a + b + c + d = 0$ 变形为：$$a + b = -(c + d)$$

如果我们先算出 $a + b$ 的所有可能和，存进一个哈希表。然后再算出 $c + d$ 的所有可能和。

Key 显然应该是 $a + b$ 的和。

第一阶段：遍历数组 nums1 和 nums2，计算所有 a + b 的和，并存入 map。Value 应该存什么？（提示：题目要求的是“元组个数”，如果 A = [1, 1], B = [0, 0]，a + b = 1 出现了几次？）

第二阶段：遍历数组 nums3 和 nums4，计算每个 c + d。